...
import { getRandomNumber, shuffleArray, playSound, PATH_API } from '../utils';
import sound from '../assets/image/sound.png';
import arrow from '../assets/image/next-arrow.png';
import SettingsContext from './SettingsContext';
import LangApi from './LangApi';

const GameBoardAudiobattle = ({ dataListWords, setFinish, setResult }) => {
  ...
  const changeStatistics = (action) => {
    if (context.user.userId) {
      LangApi.updateUserWords(
        context.user.userId,
        context.user.token,
        listWords[currentWord]._id,
        action,
        null,
        'game-3'
      );
    ...
    <>
      <div className="game-audio__board">
        <div className="word-info">
          {isChoose ? (
            <div className="word-info__inner">
              <div className="word-info__img-wrap">
                <img src={`${PATH_API}${listWords[currentWord].image}`} alt="wrap" />
              </div>
              <div className="word-info__gap">
                <button
                  type="button"
                  className="sound"
                  onClick={() => playSound(listWords[currentWord].audio)}
                >
        ...
        <div
          className={isChoose ? 'game-audio__variants disabled' : 'game-audio__variants'}
          ref={variantsBlock}
        >
          {currentVariants.map((el, index) => (
            <button type="button" onClick={(e) => userChoise(e, el)} key={index}>
              {listWords[el].wordTranslate}
            </button>
          ))}
        </div>
        <button
          type="button"
          className={isChoose ? 'game-audio__btn game-audio__btn--next' : 'game-audio__btn'}
          onClick={changeList}
        >
