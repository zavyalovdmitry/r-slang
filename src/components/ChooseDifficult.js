...
const ChooseDifficult = ({ setDifficult }) => {
  const difficultList = new Array(6).fill(0).map((_, index) => index);

  return (
    <article className="h100 choose-difficult">
      <h2 className="choose-difficult__title">Выбери сложность игры:</h2>
      <ul className="choose-difficult__list">
        {difficultList.map((el, index) => (
          <DifficultItem difficultValue={el} key={index} setDifficult={setDifficult} />
        ))}
      </ul>
    </article>
  );
