...
const GameBoardSavanna = ({ life, dataListWords, changeLife, setFinish, setResult }) => {
  ...
  const getVariants = () => {
    ...
    return arrVariants.map((el, index) => (
      <button type="button" onClick={() => userChoise(el)} key={index}>
  ...
  return (
    <>
      ...
      <div className="game-savanna__board">
        <p className="game-savanna__current-word" ref={wordElement}>
          {listWords[currentWord].word}
        </p>
