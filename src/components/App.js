...
class App extends Component {
  ...
  saveSettings = () => {
    const { user, wordTranslateVisible, textTranslateVisible, deleteWordVisible, hardWordVisible } =
      this.state;
    const { userId, token } = user;
    if (userId && token) {
      const optional = {
        wordTranslateVisible: wordTranslateVisible.value,
        textTranslateVisible: textTranslateVisible.value,
        deleteWordVisible: deleteWordVisible.value,
        hardWordVisible: hardWordVisible.value
      };
  ...
  loadSettings = () => {
    ...
    const { userId, token } = user;
    LangApi.getUserSettings(userId, token).then((settings) => {
      if (settings) {
  ...
  SetlogInUser = (userId, token, refreshToken) => {
    this.setState({ user: { userId, token, refreshToken } }, () => this.loadSettings());
  };
          ...
          <main>
            <Switch>
              ...
              <Route path="/selectedSavana">
                <Savanna
                  listDictionary={dataForGame}
                  inputDifficult={dataForGame && dataForGame[0] ? dataForGame[0].group : -1}
                />
              </Route>
