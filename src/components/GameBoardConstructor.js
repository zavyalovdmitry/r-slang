...
const GameBoardConstructor = ({
  ...
  const alphabetBoard = () => (
    <>
      {alphabet.map((el, i) => (
        <span
          className={`game-constructor__alphBoard-btn ${checkLetter(el) ? 'active' : 'outgame'}`}
          key={i}
  ...
  return (
    <article className="game-sprint__board">
      ...
      <div>{alphabetBoard()}</div>
      <div className="game-sprint__btn-block">
        <button
          type="button"
          className="game-sprint__btn game-sprint__btn--wrong"
          onClick={(e) => delLetter(e)}
        >
          удалить букву
        </button>
        <button
          type="button"
          className="game-sprint__btn game-sprint__btn--right"
          onClick={(e) => getNextWord(e)}
        >
          следующее слово
        </button>
      </div>
